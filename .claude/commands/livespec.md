...
Your project companion for living specification management. Ensures project structure is configured, specs are valid, in sync with code, and have test coverage.

## When called

You can be called in two ways:

1. **Just `/livespec`** — Run the full workflow below
2. **`/livespec [task]`** — Follow the user's lead on a specific task (e.g., `/livespec add dark mode`)

For specific tasks, determine if it needs a plan (significant feature) or can be done directly. Follow the livespec workflow from @livespec/livespec.md.
...
Update the sync report file with final results. Remove "(In Progress)" from title. Keep last 10 reports, delete older ones.
