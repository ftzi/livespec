...
describe("multiple tools", () => {
  ...
  it("creates command files for all selected tools", () => {
    ...
    expect(existsSync(join(TEST_DIR, ".cursor/prompts/livespec.md"))).toBe(
      true
    );
    expect(existsSync(join(TEST_DIR, ".windsurf/workflows/livespec.md"))).toBe(
      true
    );
  });
  ...
  it("all command files have the same content", () => {
    ...
    const cursorContent = readFileSync(
      join(TEST_DIR, ".cursor/prompts/livespec.md"),
      "utf-8"
    );
    const windsurfContent = readFileSync(
      join(TEST_DIR, ".windsurf/workflows/livespec.md"),
      "utf-8"
    );
  ...
  it("tracks created directories and files", () => {
    ...
    expect(result.created.some((f) => f.includes(".claude/commands"))).toBe(
      true
    );
    expect(result.created.some((f) => f.includes(".github/prompts"))).toBe(
      true
    );
    expect(result.created.some((f) => f.includes("livespec.md"))).toBe(true);
    expect(result.created.some((f) => f.includes("livespec.prompt.md"))).toBe(
      true
    );
  });

  /** @spec [LIV.update.detect-tools.multiple] */
...
describe("detectInstalledTools", () => {
  ...
  /** @spec [LIV.update.detect-tools.none] */
...
/** @spec [LIV.update.tools] */
describe("updateBaseFiles with tools", () => {
  beforeEach(setupTestDir);
  afterEach(cleanupTestDir);

  /** @spec [LIV.update.tools.update] */
